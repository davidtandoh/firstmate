...
| silent failure handling | a source exit with no output, whether nonzero or exit 0, publishes nothing, captures no result or adapter sidecar, and leaves the source registered for retry |
| explicit historical empty remote-reply recovery | `tests/fm-remote-reply.test.sh` proves that ordinary handling refuses an old zero-byte capture and reconciliation re-announces it; adapter-owned `recover-empty` then preserves the result and adapter bytes, binds the ingestion receipt, re-arms at the unchanged cursor, acknowledges the exact generation, and prevents another announcement; failed re-arming retains the receipt and unhandled capture for retry; repeated recovery is safe, and mismatched identity or malformed nonempty evidence is refused without acknowledgement |
