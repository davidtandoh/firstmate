...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, and the non-recursive recovery mutex: a stale or same-process mutex is reclaimed at its fixed path while a live or unreadable owner refuses.
