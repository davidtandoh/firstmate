...
| Effort | `--effort low\|medium\|high`, except `claude-opus-4-6-thinking`: spawn omits effort with a notice because native selection rejected that combination and fell back to Gemini. Requested effort stays in task metadata; `xhigh` and `max` also follow record-and-omit. Other models retain existing behavior; model names do not prove effort support. |
