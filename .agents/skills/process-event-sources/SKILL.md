...
: **When the adapter owns applying the result, run the adapter, not the generic acknowledgement below.** The `<adapter>` field of the wake decides this, and `remote-reply` is such an adapter: a captured delta is applied only by
  ...
  A historical zero-byte capture requires the adapter's explicit `recover-empty` operation, described in its header and help.
  That operation preserves the original capture and sidecar, binds recovery to the captured source and sequence, reuses the ingestion receipt and generation acknowledgement owners, and re-arms at the unchanged cursor.
  Malformed nonempty or mismatched evidence remains unhandled; never rewrite or discard evidence to silence a wake.
